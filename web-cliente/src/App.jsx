...
function MainLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col font-sans transition-colors duration-300">
      ...
      {/* ─── HEADER PREMIUM ─── */}
      <header className={`sticky top-0 z-40 transition-all duration-300 ${isScrolled ? 'bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg shadow-sm border-b border-slate-200 dark:border-slate-800' : 'bg-transparent'}`}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between h-16 sm:h-20">
            {/* Logo */}
            <div className="flex-shrink-0 flex items-center">
              <img src="/logo.png" alt="Precios Al Día" className="h-8 sm:h-10 dark:block hidden light-mode-logo" style={{ display: 'none' }} />
              <img src="/logo.png" alt="Precios Al Día" className="h-8 sm:h-10 dark:hidden" />
              <img src="/logo dark.png" alt="Precios Al Día" className="h-8 sm:h-10 hidden dark:block" />
            </div>
        ...
        <div className="max-w-4xl mx-auto text-center sm:text-left">
          ...
          {config && !config.isOpen && (
            <div className="mt-4 inline-block bg-red-900/40 backdrop-blur border border-red-400/30 px-4 py-2 rounded-xl text-red-50 font-bold text-sm shadow-xl shadow-red-900/20">
              <span className="flex items-center gap-2"><Flame size={16} /> Lo sentimos, estamos cerrados por los momentos</span>
            </div>
          )}
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-64 text-center">
              <div className="w-24 h-24 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center mb-6">
                <UtensilsCrossed size={40} className="text-slate-300 dark:text-slate-600" />
              </div>
              <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300 mb-2">Aún no hay platos aquí</h3>
              <p className="text-slate-500 max-w-xs">Nuestro chef está preparando el menú, por favor vuelve más tarde.</p>
            </div>
          ) : (
