...
export function useCart() {
    ...
    const addToCart = (product, qty = 1, size = 'Sencillo', selectedExtras = [], note = '') => {
        setCart(prev => {
            ...
            if (selectedExtras && selectedExtras.length > 0) {
                const extrasTotal = selectedExtras.reduce((sum, ext) => sum + (parseFloat(ext.price) || 0), 0);
    ...
    const cartTotal = cart.reduce((sum, item) => sum + (item.priceUsd * item.qty), 0);
