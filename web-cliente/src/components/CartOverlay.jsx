...
import { X, Minus, Plus, ShoppingBag, ArrowRight, UtensilsCrossed, Car } from 'lucide-react';
import { supabase } from '../utils/supabase';

export default function CartOverlay({ cartHooks, isOpen, onClose }) {
    const { cart, removeFromCart, updateQty, clearCart, cartTotal, cartCount } = cartHooks;
    ...
    const handleSubmitOrder = async (e) => {
        ...
        try {
            ...
            const { error } = await supabase.from('web_orders').insert([orderPayload]);
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-hidden">
            ...
            {/* Panel */}
            <div className={`absolute top-0 right-0 h-full w-full max-w-md bg-white dark:bg-slate-950 shadow-2xl flex flex-col transition-transform transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>

                {/* Header */}
                <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 shrink-0">
                    <h2 className="text-xl font-black text-slate-800 dark:text-white flex items-center gap-2">
                        {view === 'success' ? '¡Orden Enviada!' : view === 'checkout' ? 'Finalizar Pedido' : 'Tu Orden'}
                    </h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
                    ...
                    {view === 'success' && (
                        <div className="h-full flex flex-col items-center justify-center space-y-4 text-center">
                            ...
                            <h3 className="text-2xl font-black text-slate-800 dark:text-white">¡Gracias por tu pedido!</h3>
                            <p className="text-slate-500 dark:text-slate-400 max-w-[250px]">
                                Hemos recibido tu orden en cocina. Te escribiremos muy pronto por WhatsApp para confirmar los detalles del pago.
                            </p>
                        </div>
                    )}
                    ...
                    {view === 'cart' && cart.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center space-y-4 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-700 dark:text-slate-300">Tu carrito está vacío</h3>
                            <p className="text-slate-500 dark:text-slate-400">
                                Agrega algunas deliciosas opciones de nuestro menú
                            </p>
                            <button onClick={onClose} className="mt-6 px-6 py-3 bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 font-bold rounded-xl active:scale-95 transition-all">
                                Explorar Menú
                            </button>
                        </div>
                    )}
                    ...
                    {view === 'cart' && cart.length > 0 && (
                        <div className="space-y-4">
                            {cart.map(item => (
                                <div key={item.cartId} className="flex gap-4 p-4 bg-slate-50 dark:bg-slate-900/50 rounded-[20px] border border-slate-100 dark:border-slate-800">
                                    {item.image ? (
                                        <img src={item.image} alt={item.name} className="w-20 h-20 rounded-[16px] object-cover shrink-0 shadow-sm" />
                                    ) : (
                                    ...
                                    <div className="flex-1 flex flex-col justify-between py-0.5">
                                        <div>
                                            <div className="flex justify-between items-start gap-2">
                                                <h4 className="font-bold text-slate-800 dark:text-slate-200 text-[15px] leading-tight line-clamp-2">
                                                    {item.name} {item.size && item.size !== 'Sencillo' && `[${item.size}]`}
                                                </h4>
                                                <button onClick={() => removeFromCart(item.cartId)} className="text-slate-300 hover:text-red-500 transition-colors p-1 -mr-2 -mt-1">
                                                    <X size={16} />
                                                </button>
                                            </div>
                                            ...
                                            {item.note && (
                                                <p className="text-[11px] text-amber-600 dark:text-amber-500 mt-1 flex gap-1 line-clamp-1">
                                                    <span className="shrink-0 bg-amber-100 dark:bg-amber-900/30 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase">Nota</span>
                                                    {item.note}
                                                </p>
                                            )}
                                        </div>
                                        <div className="flex items-center justify-between mt-3">
                                            ...
                                            <div className="flex items-center gap-3 bg-white dark:bg-slate-800 px-2 py-1.5 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                                                <button onClick={() => updateQty(item.cartId, -1)} className="text-slate-400 hover:text-red-500 active:scale-90 transition-all">
                    ...
                    {view === 'checkout' && (
                        <form id="checkout-form" onSubmit={handleSubmitOrder} className="space-y-6">
                            ...
                            <div className="bg-red-50 dark:bg-red-900/10 p-4 rounded-[20px] border border-red-100 dark:border-red-900/20">
                                <h3 className="font-bold text-red-800 dark:text-red-400 mb-1">¡Casi listo!</h3>
                                <p className="text-sm text-red-600/80 dark:text-red-400/80 leading-relaxed">
                                    Déjanos tus datos para enviarte el total en Bolívares por WhatsApp y procesar tu pedido inmediatamente.
                                </p>
                            </div>

                            <div className="space-y-4">
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Tu Nombre</label>
                                    <input
                                        type="text"
                                        required
                                        value={name}
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">¿Para Llevar o Comer Aquí?</label>
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        >
                                            <Car size={24} className={deliveryType === 'LLEVAR' ? 'animate-bounce' : ''} />
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Notas Adicionales (Opcional)</label>
                                    <textarea
                                        value={notes}
