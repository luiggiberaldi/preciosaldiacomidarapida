...
import { Home, ShoppingCart, Store, Users, Download, FlaskConical, Key, Moon, Sun, BarChart3, WifiOff, Flame, BellRing } from 'lucide-react';
...
import { InboxView } from './views/InboxView'; // Eager loaded so the bell icon gets the state correctly
...
export default function App() {
  ...
  const pendingWebCount = webOrders.filter(o => o.status === 'pending').length;
  ...
  const TABS = [
    ...
    { id: 'inbox', label: 'Pedidos Web', icon: BellRing, badge: pendingWebCount },
  ...
  return (
    <div className="font-sans antialiased bg-slate-50 dark:bg-black h-[100dvh] flex flex-col overflow-hidden transition-colors duration-300">
        ...
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 z-[100] animate-in slide-in-from-bottom-4 fade-in duration-500 pointer-events-none">
          <div className="px-3 py-1.5 bg-slate-900/90 dark:bg-white/10 backdrop-blur-md rounded-full border border-white/10 shadow-xl flex items-center gap-2">
            ...
            <p className="text-[10px] font-semibold text-white tracking-wide">
              Licencia: <span className="text-red-400 font-bold">{demoTimeLeft}</span>
            </p>
          </div>
        </div>
      )}
      ...
      <main className={`flex-1 min-h-0 w-full max-w-md md:max-w-2xl lg:max-w-7xl mx-auto relative ${isKeyboardOpen ? 'pb-4' : 'pb-24'} flex flex-col overflow-hidden`}>
        ...
        <div className={`flex-1 flex flex-col ${activeTab === 'inbox' ? '' : 'hidden'}`}>
          <ErrorBoundary>
        ...
        <Suspense fallback={<div className="flex-1 p-4 space-y-4"><div className="skeleton h-10 w-40" /><div className="skeleton h-32" /><div className="skeleton h-48" /></div>}>
            ...
            <div data-view="clientes" className={`flex-1 flex flex-col ${activeTab === 'clientes' ? '' : 'hidden'}`}>
              <ErrorBoundary>
                <PremiumGuard featureName="Gestión de Clientes">
                  <CustomersView triggerHaptic={triggerHaptic} onNavigate={setActiveTab} />
                </PremiumGuard>
              </ErrorBoundary>
            </div>
          )}
          ...
          {(activeTab === 'cocina' || document.querySelector('[data-view="cocina"]')) && (
            <div data-view="cocina" className={`flex-1 flex flex-col ${activeTab === 'cocina' ? '' : 'hidden'}`}>
              <ErrorBoundary>
                <PremiumGuard featureName="Pantalla de Cocina / KDS">
                  <KitchenView triggerHaptic={triggerHaptic} onNavigate={setActiveTab} />
                </PremiumGuard>
              </ErrorBoundary>
            </div>
          )}
        ...
        <div className="fixed bottom-0 left-0 right-0 px-6 pb-[env(safe-area-inset-bottom)] pt-0 mb-6 max-w-md md:max-w-lg mx-auto z-30 pointer-events-none animate-in slide-in-from-bottom-4 duration-300">
          <div className="bg-slate-900/95 dark:bg-slate-950/95 backdrop-blur-xl rounded-3xl p-1.5 flex justify-between items-center shadow-2xl shadow-slate-900/30 border border-white/10 ring-1 ring-black/5 pointer-events-auto">
            ...
            {installPrompt && activeTab === 'inicio' && (
              <button onClick={() => { triggerHaptic(); handleInstall(); }} className="flex-1 flex flex-col items-center justify-center gap-1 py-3 rounded-2xl transition-all duration-300 bg-red-500 text-white shadow-md animate-pulse">
...
function TabButton({ icon, label, isActive, onClick, badge }) {
  return (
    <button onClick={onClick} className={`relative flex-1 flex flex-col items-center justify-center gap-1 py-3 rounded-2xl transition-all duration-300 ${isActive ? 'bg-red-500 text-white shadow-md' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}>
      {icon}
      {badge > 0 && (
