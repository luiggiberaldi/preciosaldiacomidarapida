...
export default function SalesView({ rates, triggerHaptic, onNavigate }) {
    ...
    const handleCheckout = async (payments, checkoutInfo = {}) => {
        ...
        const sale = {
            ...
            items: cart.map(i => ({ id: i.id, name: i.name, qty: i.qty, priceUsd: i.priceUsd, costBs: i.costBs || 0, isWeight: i.isWeight, note: i.note || '' })),
        ...
        const maxSaleNumber = existingSales.reduce((mx, s) => Math.max(mx, s.saleNumber || 0), 0);
    ...
    return (
        <div className="flex-1 min-h-0 flex flex-col bg-slate-50 dark:bg-slate-950 p-2 sm:p-4 overflow-hidden relative">
            ...
            <CartPanel
                ...
                onEditNote={(item) => { triggerHaptic && triggerHaptic(); setNotePending(item); }}
                triggerHaptic={triggerHaptic}
            />
