...
import { Package, Plus, Trash2, X, Store, Tag, Pencil, Banknote, Search, ChevronLeft, ChevronRight, Settings, AlertTriangle, Box, Globe, UploadCloud } from 'lucide-react';
...
export const ProductsView = ({ rates, triggerHaptic }) => {
    ...
    const autoSyncWebCatalog = async (updatedProducts) => {
        try {
            const activeProducts = updatedProducts.filter(p => p.available !== false).map(p => ({
                id: p.id,
                name: p.name,
            ...
            if (activeProducts.length > 0) {
                await webSupabase.from('web_catalog').upsert(activeProducts, { onConflict: 'id' });
            }
            ...
            if (activeIds.length > 0) {
                await webSupabase.from('web_catalog').delete().not('id', 'in', `(${activeIds.join(',')})`);
            } else {
    ...
    const handleClose = () => {
        ...
        setDescription(''); setPrepTime(10); setAvailable(true); setSizes([]); setExtras([]);
    };
    ...
    const handlePublishWeb = async () => {
        if (!products.length) return showToast('No hay productos para publicar', 'warning');

        triggerHaptic && triggerHaptic();
        if (!window.confirm('¿Publicar el menú actual en la App Web para clientes? Esto reemplazará el menú en línea.')) return;

        setIsPublishing(true);
        try {
            // First, get all web-ready products (that are active)
            const activeProducts = products.filter(p => p.available !== false).map(p => ({
                id: p.id,
                name: p.name,
                description: p.description || '',
                price_usd: p.priceUsdt || 0,
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950 p-3 sm:p-6 pb-8 overflow-y-auto">
            ...
            <div className="shrink-0 mb-3 space-y-2">
                <div className="flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2 min-w-0">
                        ...
                        <h2 className="text-lg sm:text-2xl font-black text-slate-800 dark:text-white tracking-tight truncate">Menú</h2>
                    </div>
                    <div className="flex items-center gap-1 shrink-0">
                        ...
                        {products.length > 0 && (
                            <button
                                ...
                                className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl shadow-md shadow-blue-500/20 transition-all active:scale-95 font-bold text-sm disabled:opacity-50" title="Publicar a la Web">
                                {isPublishing ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : <UploadCloud size={16} strokeWidth={2.5} />}
                                <span className="hidden sm:inline">Publicar Web</span>
                            </button>
                        )}
                        <button onClick={() => { triggerHaptic && triggerHaptic(); setIsModalOpen(true); }}
                            className="flex items-center gap-1.5 px-3 py-2 bg-red-500 text-white rounded-xl shadow-md shadow-red-500/20 transition-all active:scale-95 font-bold text-sm" title="Agregar">
                ...
                <>
                    ...
                    {activeCategory === 'bajo-stock' && (
                        <div className="flex items-center justify-between bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 px-3 py-2 rounded-xl mb-3 shrink-0">
                            <span className="text-xs font-bold text-red-600 dark:text-red-400">Mostrando productos con stock bajo</span>
                            <button onClick={() => handleSetActiveCategory('todos')} className="text-xs font-bold text-red-500 hover:text-amber-700 transition-colors flex items-center gap-1">
                                × Ver todos
                            </button>
                        </div>
                    )}
            ...
            <Modal isOpen={isDeleteAllModalOpen} onClose={() => { setIsDeleteAllModalOpen(false); setDeleteAllConfirmText(''); }} title="⚠️ Borrado de Menú">
