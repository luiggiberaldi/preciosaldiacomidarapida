...
export const InboxView = ({ rates, storeConfig }) => {
    ...
    if (loading) {
        return <div className="p-4 flex justify-center"><div className="animate-pulse w-8 h-8 bg-red-500 rounded-full"></div></div>;
    }
            ...
            <div className="bg-gradient-to-r from-red-600 to-red-500 text-white p-4 shadow-lg sticky top-0 z-10">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-red-100 text-sm opacity-90">Órdenes recibidas desde la página web</p>
                    </div>
                </div>
            </div>
            ...
            <div className="p-4 space-y-6">
                ...
                <section>
                    ...
                    {pendingOrders.length === 0 ? (
                        <div className="text-center py-8 bg-gray-50 rounded-2xl border border-gray-100">
                            ...
                            <p className="text-gray-500 font-medium">No hay pedidos nuevos entrantes.</p>
                        </div>
                    ) : (
                ...
                <section>
                    ...
                    {confirmedOrders.length === 0 ? (
                        <p className="text-gray-400 text-sm italic ml-7">Ningún pedido pendiente de pago.</p>
                    ) : (
                        <div className="space-y-4">
...
const OrderCard = ({ order, rates, isConfirmed, actions }) => {
    return (
        <div className={`bg-white rounded-2xl p-4 shadow-sm border ${isConfirmed ? 'border-green-100' : 'border-orange-100'}`}>
            <div className="flex justify-between items-start mb-3">
                <div>
                    <h3 className="font-bold text-gray-800 text-lg leading-tight">{order.customer_name}</h3>
                    <p className="text-gray-500 text-sm font-mono mt-0.5">{order.customer_phone}</p>
                </div>
                <div className="text-right">
                    <p className="font-bold text-gray-800 text-lg leading-tight">${order.total_usd.toFixed(2)}</p>
                    <p className="text-gray-400 text-xs">
                        Bs {rates?.euro?.price ? formatBs(order.total_usd * rates.euro.price) : '...'}
                    </p>
                </div>
            </div>
            ...
            <div className="text-xs text-gray-400 flex justify-between items-center">
                ...
                <span className={`px-2 py-0.5 rounded-full font-medium ${isConfirmed ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}>
