        ...
        <div className="min-h-screen bg-slate-950 text-white p-3 sm:p-6 space-y-3 sm:space-y-4">
            ...
            {progress && (
                <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700">
                    ...
                    <div className="h-1.5 bg-slate-900 rounded-full overflow-hidden">
                        <div className="h-full bg-gradient-to-r from-indigo-500 to-red-500 rounded-full transition-all duration-500"
            ...
            {summary && (
                <div className="grid grid-cols-4 gap-1.5 sm:gap-2">
                    ...
                    <div className="bg-amber-950/50 rounded-xl p-2 sm:p-3 text-center border border-amber-800/30">
                        <p className="text-lg sm:text-2xl font-black text-red-400">{passedSuites}</p>
                        <p className="text-[7px] sm:text-[9px] text-red-500 uppercase font-bold">Pass</p>
                    </div>
            ...
            {summary && (
                <div className={`flex items-center justify-center gap-3 p-3 rounded-xl border ${failedSuites === 0
                    ...
                    <div>
                        <p className={`text-sm sm:text-lg font-black ${failedSuites === 0 ? 'text-red-400' : failedSuites <= 3 ? 'text-red-400' : 'text-rose-400'}`}>
                    ...
                    <div className="p-2 sm:p-3 space-y-1">
                        {summary.dayStats.dailyStats.map((d, i) => (
                            <div key={i} className="flex items-center gap-2 sm:gap-3 px-2 sm:px-3 py-1.5 bg-slate-900/40 rounded-lg text-[10px] sm:text-xs">
                                ...
                                <span className="text-red-400 font-bold">${d.revenue.toFixed(2)}</span>
                                {d.fiado > 0 && <span className="text-red-400/70 text-[9px] ml-auto">fiado ${d.fiado.toFixed(2)}</span>}
                            </div>
                        ))}
                        <div className="flex items-center gap-3 px-3 py-2 bg-violet-900/20 rounded-lg text-xs border border-violet-700/20 mt-1">
                            ...
                            <span className="text-red-400 font-black">${summary.dayStats.totalRevenue.toFixed(2)}</span>
                            {summary.dayStats.totalFiado > 0 && <span className="text-red-400 ml-auto">Fiado: ${summary.dayStats.totalFiado.toFixed(2)}</span>}
                        </div>
                    </div>
                </div>
            )}
                                ...
                                <button
                                    ...
                                    className={`px-2 py-1 rounded-lg text-[8px] sm:text-[9px] font-black uppercase transition-all ${logFilter === f
                                        ? f === 'error' ? 'bg-rose-600 text-white'
                                            : f === 'success' ? 'bg-red-600 text-white'
                                                : f === 'warn' ? 'bg-red-500 text-white'
                ...
                <div className="bg-slate-900 rounded-xl border border-slate-700 overflow-hidden">
                    ...
                    <div className="max-h-40 sm:max-h-60 overflow-y-auto p-1.5 sm:p-2 space-y-0.5 sm:space-y-1">
                        {summary.suites.map((s, i) => (
                            <div key={i} className={`flex items-center gap-1.5 sm:gap-2 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg text-[9px] sm:text-xs ${s.status === 'passed' ? 'bg-amber-950/30' :
                                s.status === 'failed' ? 'bg-rose-950/30' :
                                    s.status === 'skipped' ? 'bg-slate-800' : 'bg-slate-800'
                                }`}>
                                {s.status === 'passed' ? <CheckCircle2 size={10} className="text-red-500 shrink-0" /> :
                                ...
                                <span className={`font-bold shrink-0 ${s.status === 'passed' ? 'text-red-400' :
