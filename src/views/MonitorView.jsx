...
export default function MonitorView({
  ...
  const spread =
    rates.euro.price > 0 && rates.bcv.price > 0
      ? ((rates.euro.price - rates.bcv.price) / rates.bcv.price) * 100
      : 0;
  ...
  const renderChange = (change) => {
    ...
    return (
      <span
        className={`text-[10px] font-bold px-2 py-1 rounded-lg flex items-center gap-1 border ${isPositive ? "bg-amber-50 text-red-600 border-amber-100 dark:bg-amber-900/20 dark:text-red-400 dark:border-amber-900/30" : "bg-rose-50 text-rose-600 border-rose-100 dark:bg-rose-900/20 dark:text-rose-400 dark:border-rose-900/30"}`}
      >
    ...
    try {
      ...
      const canvas = await html2canvas(kioskRef.current, {
        ...
        ignoreElements: (element) =>
          element.hasAttribute("data-hide-on-capture"),
      });
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center -mt-8">
          ...
          <p className="text-xl sm:text-3xl text-slate-500 font-mono font-medium tracking-widest mt-2">
            1 USD = BS
          </p>
        </div>
        ...
        <div className="w-full max-w-md bg-slate-900/50 backdrop-blur-sm rounded-[2rem] border border-slate-800/50 p-6 sm:p-8 flex justify-between items-center mb-8">
          <div className="text-center w-1/2 border-r border-slate-800 pr-4">
            <p className="text-xs sm:text-sm font-bold uppercase text-slate-500 tracking-wider mb-2">
              BCV OFICIAL
            </p>
          ...
          <div className="text-center w-1/2 pl-4">
            ...
            <p className="text-2xl sm:text-4xl font-mono font-bold text-white">
              {formatExactRate(rates.euro.price)}
            </p>
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-150px)] overflow-hidden justify-between py-2 relative">
      {/* HEADER */}
      <header
        className="flex items-center justify-between pt-[env(safe-area-inset-top)] pb-2 px-3 sm:px-4 shrink-0"
      ...
      >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          ...
          <button
            ...
            disabled={loading}
            className={`p-2 sm:p-2.5 rounded-2xl text-slate-900 shadow-lg shadow-brand/10 border border-transparent transition-all active:scale-95 ${loading ? "bg-slate-100 dark:bg-slate-800 text-slate-300 cursor-not-allowed" : "bg-brand hover:bg-brand-light"}`}
          >
            <RefreshCw
              size={18}
      ...
      {isOldData && (
        <div className="mx-3 sm:mx-4 px-4 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 rounded-xl flex items-center justify-center gap-2 animate-in slide-in-from-top-2 shrink-0">
          ...
          <p className="text-xs font-bold text-amber-700 dark:text-red-500">
      ...
      <div className="flex-1 flex flex-col justify-center gap-4 min-h-0">
        ...
        <div className="relative group shrink-0">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-[2rem] p-6 shadow-2xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-800 overflow-hidden">
            ...
            <div className="flex items-baseline gap-1 mb-4 select-none">
              <span className="text-2xl text-slate-300 dark:text-slate-600 font-bold font-sans transform -translate-y-4">
                $
              </span>
              <div className="text-[12vw] sm:text-[4rem] lg:text-[3.5rem] xl:text-[4rem] leading-none font-black text-slate-900 dark:text-white tracking-tighter font-mono">
                ...
                <span className="text-3xl text-slate-400 dark:text-slate-600">
                  ,{formatExactRate(rates.bcv.price).split(",")[1]}
                </span>
              </div>
            ...
            <div className="flex items-center gap-3 pt-4 border-t border-slate-50 dark:border-slate-800">
              <span className="text-xs font-medium text-slate-400 dark:text-slate-500 truncate">
                Fuente: {rates.bcv.source || "BCV Oficial"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 shrink-0">
          <RateCardMini
            title="Dolar BCV Oficial"
            price={rates.bcv.price}
            change={rates.bcv.change}
            icon="🏛️"
            formatVES={formatExactRate}
            renderChange={renderChange}
            symbol="Bs / $"
          />
          <RateCardMini
            title="Euro BCV Oficial"
            price={rates.euro.price}
            change={rates.euro.change}
            icon="🇪🇺"
            formatVES={formatExactRate}
            renderChange={renderChange}
            symbol="Bs / €"
...
function RateCardMini({
  ...
  return (
    <div className="bg-white dark:bg-slate-900 p-5 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 duration-300">
      ...
      <div className="text-xl font-black text-slate-800 dark:text-white tracking-tight font-mono">
        {formatVES(price)}
      </div>
