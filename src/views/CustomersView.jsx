...
import { Users, Plus, Search, User, X, Trash2, Pencil, Phone, RefreshCw, Save, ArrowDownRight, ArrowUpRight, Clock, CheckCircle2, ShoppingBag, CreditCard } from 'lucide-react';
                    ...
                    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white dark:bg-slate-900 w-full max-w-sm rounded-t-3xl sm:rounded-3xl shadow-xl overflow-hidden animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-200">
                            <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                                <h3 className={`text-xl font-black ${transactionModal.type === 'ABONO' ? 'text-red-500' : 'text-red-500'}`}>
                                ...
                                <div>
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        >
                                            ...
                                            <span className={currencyMode === 'USD' ? 'text-red-500' : 'text-slate-400'}>$</span>
                                        </button>
                                    </div>
        ...
        >
            ...
            <div className="text-right shrink-0">
                {customer.deuda > 0 ? (
                    ...
                    <>
                        <p className="text-sm font-black text-red-500 leading-tight">+${formatUsd(customer.favor)}</p>
                        {bcvRate > 0 && <p className="text-[10px] font-bold text-red-400/70">+{formatBs(customer.favor * bcvRate)} Bs</p>}
                    </>
                ) : (
...
function CustomerDetailSheet({ customer, isOpen, onClose, onDeuda, onAbono, onReset, onEdit, onDelete, bcvRate, sales, onNavigate }) {
    ...
    const totalSpend = sales ? sales.reduce((sum, s) => sum + (s.totalUsd || 0), 0) : 0;

    let badge = null;
    if (salesCount >= 10 || totalSpend >= 50) badge = { label: 'VIP', bg: 'bg-red-500', icon: '⭐' };
    else if (salesCount >= 3) badge = { label: 'Frecuente', bg: 'bg-blue-500', icon: '🔥' };

    // Phase 10: 1-Click Reorder
    const lastSaleWithItems = sales?.find(s => s.items && s.items.length > 0 && s.tipo !== 'COBRO_DEUDA' && s.tipo !== 'VENTA_FIADA');

    const handleReorder = () => {
        if (!lastSaleWithItems) return;
        window.dispatchEvent(new CustomEvent('inject_cart', { detail: lastSaleWithItems.items }));
                ...
                <div className="px-5 pb-6 space-y-5">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                ...
                                {badge && (
                                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-full text-white shadow-sm flex items-center gap-1 ${badge.bg}`}>
                    ...
                    <div className="flex gap-2">
                        {customer.deuda > 0 ? (
                            ...
                            <div className="flex-1 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/30 rounded-xl px-3 py-2.5 text-center">
                                <p className="text-[10px] font-bold text-red-400 uppercase">A favor</p>
                                <p className="text-lg font-black text-red-500">+${formatUsd(customer.favor)}</p>
                                {bcvRate > 0 && <p className="text-[10px] font-bold text-red-400/70">+{formatBs(customer.favor * bcvRate)} Bs</p>}
                            </div>
                        ) : (
                    ...
                    {lastSaleWithItems && (
                        <div className="pt-2">
                            ...
                            <p className="text-[10px] text-center text-slate-400 mt-2 font-medium">Último pedido: {lastSaleWithItems.items.map(i => i.name).join(', ')}</p>
                        </div>
                    )}
                                ...
                                {sales.slice(0, 10).map(sale => {
                                    ...
                                    return (
                                        <div key={sale.id} className="flex items-start gap-2.5 py-2 px-2 bg-slate-50 dark:bg-slate-950 rounded-xl">
                                            <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${isCobro ? 'bg-amber-100 dark:bg-amber-900/30' : isFiada ? 'bg-amber-100 dark:bg-amber-900/30' : 'bg-blue-100 dark:bg-blue-900/30'}`}>
                                                {isCobro ? <ArrowUpRight size={14} className="text-red-500" /> : isFiada ? <CreditCard size={14} className="text-red-500" /> : <ShoppingBag size={14} className="text-blue-500" />}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-start">
                                                    ...
                                                    <span className={`text-xs font-black ${isCobro ? 'text-red-500' : isFiada ? 'text-red-500' : 'text-slate-700 dark:text-white'}`}>
                                                ...
                                                {sale.fiadoUsd > 0 && (
                                                    <p className="text-[10px] text-red-500 font-bold mt-0.5">Deuda: ${formatUsd(sale.fiadoUsd)}</p>
                                                )}
