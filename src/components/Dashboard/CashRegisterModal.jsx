...
export default function CashRegisterModal({
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl border border-slate-100 dark:border-slate-800 overflow-hidden animate-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]"
      ...
      >
        ...
        <div className="p-6 overflow-y-auto space-y-6">
              ...
              <div className="space-y-2">
                {Object.entries(paymentBreakdown).map(([method, data]) => {
                  ...
                  return (
                    <div
                      key={method}
                      className="flex items-center justify-between p-3.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl hover:border-indigo-300 dark:hover:border-indigo-500/50 transition-colors"
                    >
          ...
          <div className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800/50 rounded-xl p-4 flex items-start gap-3 text-sm text-amber-800 dark:text-red-400 font-medium">
            ...
            <p>
              Al confirmar, se generará el PDF resumen del día y{" "}
              <strong className="font-black">
                las ventas de hoy se archivarán para liberar la caja
              </strong>
              . ¡No olvides enviar el reporte a tu administrador!
            </p>
          </div>
        </div>
