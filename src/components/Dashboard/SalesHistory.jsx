      ...
      <div className="space-y-3">
        {recentSales.map((s) => {
          ...
          return (
            <div
              key={s.id}
              className={`rounded-xl border transition-all ${isCanceled ? "bg-red-50/50 border-red-100/50 dark:bg-red-900/10 dark:border-red-900/20" : "bg-slate-50 dark:bg-slate-800/50 border-slate-200/60 dark:border-slate-700/60"} overflow-hidden`}
            >
              <div
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${isCanceled ? "bg-red-100 opacity-50" : "bg-white dark:bg-slate-700 shadow-sm"}`}
                >
                  {isCanceled ? (
                    <Ban size={20} className="text-red-400" />
                  ) : PayMethodIcon ? (
                    <PayMethodIcon size={20} className="text-slate-500" />
                  ) : (
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-sm font-bold flex items-center gap-1.5 truncate ${isCanceled ? "line-through text-slate-400" : "text-slate-800 dark:text-slate-200"}`}
                  >
                    {s.customerName || "Consumidor Final"}{" "}
                    {s.tipo === "VENTA_FIADA" && (
                      <span className="text-[9px] bg-amber-100 text-red-600 px-1 rounded uppercase">
                        Fiado
                      </span>
                    )}
                  </p>
                ...
                <div className="text-right shrink-0">
                  <p
                    className={`text-sm font-black ${isCanceled ? "text-slate-400" : "text-slate-800 dark:text-white"}`}
                  >
              ...
              {isExpanded && (
                <div className="px-3 pb-3 pt-1 border-t border-slate-200 dark:border-slate-700/50 text-sm animate-in fade-in slide-in-from-top-1">
                  {s.items && s.items.length > 0 ? (
                    <div className="space-y-1 mb-3 pt-2">
                      ...
                      {s.items.map((item, i) => (
                        <div
                          key={i}
                          className={`flex justify-between items-center text-xs ${isCanceled ? "text-slate-400 line-through" : "text-slate-600 dark:text-slate-300"}`}
                        >
                    ...
                    <p className="text-xs text-slate-400 mb-3 pt-2">
                      Pago de Deudas (Sin productos)
                    </p>
                  )}
                  ...
                  <div className="flex justify-between text-[10px] font-medium text-slate-400 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-lg p-2 mb-3">
                    <div>
                      Ref: {formatBs(s.totalBs)} Bs @{" "}
                      {formatBs(s.rate || bcvRate)}
                    </div>
                    {s.changeUsd > 0 && (
                      <div className="text-red-500">
                        Vuelto: ${s.changeUsd.toFixed(2)}
                      </div>
                    )}
                  </div>
