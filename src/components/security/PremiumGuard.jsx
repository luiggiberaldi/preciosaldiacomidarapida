...
export default function PremiumGuard({
  ...
  const { deviceId, isPremium, loading, unlockApp, activateDemo, demoUsed } =
    useSecurity();
  ...
  const handleActivateDemo = async () => {
    ...
    if (result.success) {
      setMessageModal({
        ...
        title: "🎉 ¡Demo Activada!",
        content:
          "Disfruta de todas las funciones premium durante 7 días. Aprovecha al máximo la herramienta.",
      });
      ...
      setMessageModal({
        ...
        content:
          "El periodo de prueba ya fue utilizado en este dispositivo. Contacta soporte para adquirir tu licencia.",
      });
    }
  };
  ...
  if (isShop) {
    ...
    benefits = (
      <>
        <BenefitItem
          icon={<Store size={15} className="text-indigo-500" />}
          text="Menú ilimitado de productos"
        />
        <BenefitItem
          icon={<CreditCard size={15} className="text-blue-500" />}
          text="Sistema de Ventas y POS completo"
        />
        <BenefitItem
          icon={<BarChart3 size={15} className="text-purple-500" />}
          text="Reportes históricos con PDF"
        />
        <BenefitItem
        ...
        />
        <BenefitItem
          icon={<Search size={15} className="text-red-500" />}
          text="Categorías y búsqueda por voz"
        />
        <BenefitItem
          icon={<Volume2 size={15} className="text-rose-500" />}
          text="Sonidos inmersivos"
        />
      </>
    );
  } else {
    ...
    message = (
      <span>
        Acceso exclusivo a <strong>{featureName}</strong> para miembros.
      </span>
    );
    Icon = Lock;
    ...
    benefits = (
      <>
        <BenefitItem
          ...
          text="Gestión completa del menú"
        />
        <BenefitItem
          icon={<Star size={15} className="text-red-500" />}
          text="Punto de Venta integrado"
        />
        <BenefitItem
          icon={<BarChart3 size={15} className="text-indigo-500" />}
          text="Reportes con descarga PDF"
        />
        <BenefitItem
          icon={<Bell size={15} className="text-rose-500" />}
          text="Alertas de stock y cierre de caja"
        />
        <BenefitItem
          icon={
    ...
    <div className="flex flex-col items-center justify-center h-full p-2 text-center overflow-hidden px-4">
      ...
      <div className="w-full max-w-[320px] sm:max-w-sm max-h-[95%] overflow-hidden rounded-[2rem] p-4 relative bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-2xl shadow-slate-200/50 dark:shadow-none">
        ...
        <div className="bg-slate-50 dark:bg-slate-800 rounded-xl p-2 mb-3 border border-slate-100 dark:border-slate-700/50">
          <p className="text-[10px] uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-0.5 font-bold leading-tight">
            Tu ID de Instalación
          </p>
        ...
        >
          <p className="text-[10px] text-slate-400 dark:text-slate-500 mb-1.5 font-bold uppercase tracking-wide leading-tight">
          ...
          <div className="text-center py-4">
            ...
            <button
              onClick={() => {
                ...
                if (messageModal.title.includes("Activada"))
