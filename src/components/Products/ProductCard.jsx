...
export default function ProductCard({
    ...
    return (
        <div className={`bg-white dark:bg-slate-900 rounded-2xl shadow-sm border flex flex-col overflow-hidden group transition-all ${isUnavailable ? 'opacity-50 border-slate-300 dark:border-slate-700 grayscale' : 'border-slate-100 dark:border-slate-800'}`}>
            {/* Image */}
            ...
            <div className="p-3 flex flex-col flex-1">
                <h3 className="font-bold text-slate-700 dark:text-slate-200 text-[13px] leading-tight line-clamp-2 mb-1">{p.name}</h3>

                {/* Description */}
                {p.description && (
                    <p className="text-[10px] text-slate-400 leading-tight line-clamp-2 mb-2">{p.description}</p>
                )}
                ...
                {(p.sizes || []).length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                        {p.sizes.map(s => (
                            <span key={s.id} className="text-[9px] font-bold bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 px-1.5 py-0.5 rounded">
                ...
                {(p.extras || []).length > 0 && (
                    <p className="text-[9px] font-bold text-green-500 mt-1">
                        🧀 {p.extras.length} extra{p.extras.length > 1 ? 's' : ''} disponible{p.extras.length > 1 ? 's' : ''}
                    </p>
                )}
            </div>
            ...
            <div className="flex border-t border-slate-100 dark:border-slate-800">
                <button onClick={() => onEdit(p)} className="flex-1 py-1.5 flex items-center justify-center text-slate-300 dark:text-slate-600 hover:text-red-500 transition-colors"><Pencil size={12} /></button>
