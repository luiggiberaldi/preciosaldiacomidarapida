...
export const ProductShareModal = ({
  ...
  const generateMessage = () => {
    ...
    if (config.showRefEuro) {
      ...
      lines.push(
        `Ref. Euro: €${formatUsd(refEur).replace("$", "").replace("€", "")}`,
      );
    }
    ...
    try {
      ...
      if (navigator.share && product.image) {
        const imageFile = dataURLtoFile(
          product.image,
          `${product.name.replace(/\s+/g, "_")}.webp`,
        );
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <button
              onClick={() => applyPreset("market")}
              className="flex-1 py-2 px-3 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 rounded-xl text-xs font-bold border border-indigo-100 dark:border-indigo-800 hover:bg-indigo-100 transition-colors"
            >
              🚀 Mercado
            </button>
            <button
              onClick={() => applyPreset("fiscal")}
              className="flex-1 py-2 px-3 bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-xl text-xs font-bold border border-slate-200 dark:border-slate-700 hover:bg-slate-100 transition-colors"
            >
              🏛️ Fiscal
            </button>
            <button
              onClick={() => applyPreset("efectivo")}
              className="flex-1 py-2 px-3 bg-amber-50 dark:bg-amber-900/20 text-red-600 dark:text-red-400 rounded-xl text-xs font-bold border border-amber-100 dark:border-amber-800 hover:bg-amber-100 transition-colors"
            >
              💵 Efectivo
            </button>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {Object.keys(config).map((key) => (
            ...
            >
              {key
                .replace("show", "")
                .replace("RefBcv", "DolarBcv")
                .replace("RefEuro", "EuroBcv")}
            </button>
          ))}
        </div>
        ...
        <div className="bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
          <p className="text-[10px] text-slate-400 mb-1 uppercase font-bold">
