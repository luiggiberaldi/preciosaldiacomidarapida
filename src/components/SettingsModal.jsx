...
import PremiumGuard from "./security/PremiumGuard";
    ...
    try {
      ...
      const backupData = {
        ...
        data: {
          ...
          catalog_custom_usdt_price: localStorage.getItem(
            "catalog_custom_usdt_price",
          ),
          catalog_show_cash_price: localStorage.getItem(
  ...
  const handleFileChange = (event) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (
          !json.data ||
          (!json.data.my_products_v1 && !json.data.my_accounts_v2)
        ) {
        ...
        if (json.data.my_products_v1) {
          await storageService.setItem(
            "my_products_v1",
            typeof json.data.my_products_v1 === "string"
        ...
        if (json.data.street_rate_bs)
          localStorage.setItem("street_rate_bs", json.data.street_rate_bs);
        if (json.data.catalog_use_auto_usdt)
          localStorage.setItem(
            "catalog_use_auto_usdt",
            json.data.catalog_use_auto_usdt,
          );
        if (json.data.catalog_custom_usdt_price)
          localStorage.setItem(
            "catalog_custom_usdt_price",
            json.data.catalog_custom_usdt_price,
          );
        if (json.data.catalog_show_cash_price)
          localStorage.setItem(
            "catalog_show_cash_price",
            json.data.catalog_show_cash_price,
          );
        if (json.data.monitor_rates_v12)
          localStorage.setItem(
            "monitor_rates_v12",
            json.data.monitor_rates_v12,
          );
        ...
        <div className="p-5 space-y-3 overflow-y-auto">
          ...
          <div className="mt-3 p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 rounded-xl">
            ...
            <div className="flex items-center justify-between gap-2">
              <p className="font-mono text-xs font-black text-slate-600 dark:text-slate-300 select-all">
                {deviceId || "..."}
              </p>
              <button
                onClick={() => {
