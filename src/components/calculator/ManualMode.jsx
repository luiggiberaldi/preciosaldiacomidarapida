...
export const ManualMode = ({
  ...
  return (
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950/50">
      ...
      <div
        className={`flex-1 overflow-y-auto ${isKeyboardOpen ? "p-2" : "p-4 sm:p-6"} scrollbar-hide`}
      >
        <div
          className={`transition-all duration-300 ${isKeyboardOpen ? "space-y-2 pb-2" : "space-y-8 pb-40"}`}
        >
              ...
              {history.map((item) => (
                ...
                >
                  ...
                  <span className="text-[10px] font-bold text-slate-600 dark:text-slate-300">
                    {item.in} {item.from}{" "}
                    <span className="text-slate-300 mx-0.5">→</span> {item.out}{" "}
                    {item.to}
                  </span>
                </button>
              ))}
        ...
        <div className="space-y-5">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2 block flex items-center gap-2">
              Vista Previa{" "}
              <span className="text-[9px] font-normal normal-case text-slate-400 ml-auto opacity-70">
                (Ejemplo)
              </span>
            </label>
      ...
      >
        <div className="text-center py-2">
          <p className="text-sm text-slate-600 dark:text-slate-300 mb-4 leading-relaxed">
            ...
            <strong>PreciosAlDía Comida Rápida Business</strong>.
          </p>
          ...
          <button
            onClick={() => {
              const msg = `Hola! Me interesa la licencia Premium de PreciosAlDía Comida Rápida para enviar cotizaciones por WhatsApp.`;
              window.open(
                `https://wa.me/584124051793?text=${encodeURIComponent(msg)}`,
                "_blank",
              );
            }}
            className="w-full py-3 bg-[#10B981] text-white font-bold rounded-xl shadow-lg shadow-red-500/20 active:scale-95 transition-transform text-sm"
          >
            Solicitar Licencia
          </button>
        </div>
