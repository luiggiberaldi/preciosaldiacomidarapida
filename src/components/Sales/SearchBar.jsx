      ...
      {searchResults.length > 0 && (
        <div className="absolute top-full mt-2 left-0 right-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl z-20 overflow-hidden">
          {searchResults.map((p, index) => {
            const isLowStock =
              (p.stock ?? 0) <= (p.lowStockAlert ?? 5) && (p.stock ?? 0) >= 0;
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0 text-left">
                  <p
                    className={`text-sm font-bold truncate leading-tight ${isSelected ? "text-amber-700 dark:text-red-400" : "text-slate-800 dark:text-slate-100"}`}
                  >
                    {p.name}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                    {catInfo &&
                      ...
                      catColor && (
                        <span
                          className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${catColor}`}
                        >
                          {catInfo.label}
                        </span>
                      )}
          ...
          <div className="bg-slate-50 dark:bg-slate-950 p-2 text-center border-t border-slate-100 dark:border-slate-800">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center justify-center gap-2">
              Navega con flechas{" "}
              <span className="px-1 bg-slate-200 dark:bg-slate-800 rounded">
                ↓
              </span>
              <span className="px-1 bg-slate-200 dark:bg-slate-800 rounded">
                ↑
        ...
        <div className="absolute top-full mt-2 left-0 right-0 z-30 bg-white dark:bg-slate-900 border-2 border-indigo-200 dark:border-indigo-800 rounded-2xl shadow-2xl shadow-indigo-500/10 overflow-hidden">
          <div className="p-3 bg-indigo-50 dark:bg-indigo-900/20 border-b border-indigo-100 dark:border-indigo-800">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-black text-indigo-700 dark:text-indigo-300 uppercase tracking-wider">
                  ¿Cómo lo vendes?
                </p>
                <p className="text-[11px] text-indigo-500/70 dark:text-indigo-400/50 font-medium mt-0.5">
                  {hierarchyPending.name}
                </p>
              </div>
              <button
                onClick={() => setHierarchyPending(null)}
          ...
          <div className="grid grid-cols-2 gap-3 p-3">
            <button
              onClick={() => addToCart(hierarchyPending, null, "package")}
            ...
            <button
              onClick={() => addToCart(hierarchyPending, null, "unit")}
            ...
            >
              ...
              <span className="text-sm font-black text-red-600 dark:text-red-400">
                ${hierarchyPending.unitPriceUsd?.toFixed(2)}
              </span>
