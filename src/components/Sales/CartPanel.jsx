...
const SwipeableCartItem = ({ item, effectiveRate, updateQty, removeFromCart, onEditNote }) => {
    ...
    const handleTouchMove = (e) => {
        ...
        if (diff < 0) { // Solo desliza hacia la izquierda
        ...
        <div className="relative overflow-hidden rounded-xl sm:rounded-2xl">
            ...
            >
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0 pr-1 cursor-pointer" onClick={() => onEditNote && onEditNote(item)}>
                        ...
                        <div className="flex items-center gap-1 sm:gap-2 flex-wrap mt-1">
                            <p className="text-[10px] sm:text-[11px] font-black text-red-600 bg-amber-50 dark:bg-amber-900/30 px-1 sm:px-1.5 rounded">${item.priceUsd.toFixed(2)}</p>
                ...
                <div className="flex flex-col items-end shrink-0 gap-1.5 sm:gap-2">
                    ...
                    <div className="flex items-center bg-slate-50 dark:bg-slate-800 rounded-lg p-0.5 border border-slate-100 dark:border-slate-700">
                        <button onClick={(e) => { e.stopPropagation(); updateQty(item.id, item.isWeight ? -0.1 : -1); }} className="w-7 sm:w-8 h-7 sm:h-8 flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors rounded-l-md active:bg-slate-200 dark:active:bg-slate-700"><Minus size={14} strokeWidth={3} /></button>
                        <span className="w-10 sm:w-12 text-center font-black text-slate-700 dark:text-white text-[11px] sm:text-xs">{qtyDisplay}</span>
                        <button onClick={(e) => { e.stopPropagation(); updateQty(item.id, item.isWeight ? 0.1 : 1); }} className="w-7 sm:w-8 h-7 sm:h-8 flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors rounded-r-md active:bg-slate-200 dark:active:bg-slate-700"><Plus size={14} strokeWidth={3} /></button>
                    </div>
                </div>
                {/* Botón clásico X (oculto en móviles x el touch, visible hover en PC) */}
                <button onClick={(e) => { e.stopPropagation(); removeFromCart(item.id); }} className="absolute -top-1 -right-1 sm:top-2 sm:right-2 p-1.5 bg-red-50 dark:bg-red-900/40 text-red-500 sm:bg-transparent sm:text-slate-300 sm:hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity rounded-full sm:rounded-lg">
        ...
        <div className="flex-1 min-h-0 flex flex-col bg-white dark:bg-slate-900 rounded-2xl sm:rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <div className="shrink-0 p-3 sm:p-4 bg-white dark:bg-slate-900 border-t border-slate-200 dark:border-slate-800 rounded-b-2xl sm:rounded-b-3xl space-y-2 sm:space-y-3">
                <div className="flex justify-between items-end px-1 sm:px-0">
                    ...
                    <div className="text-right flex items-center gap-3 sm:block w-full sm:w-auto justify-between">
                        <div className="flex flex-col items-start sm:items-end">
                            <span className="text-[10px] font-black text-red-600 dark:text-red-500 tracking-widest uppercase sm:hidden">Total (Ref)</span>
                ...
                <div className="hidden sm:flex justify-between items-center px-4 py-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-800/50 rounded-xl">
                    <span className="text-[11px] font-black text-red-600 dark:text-red-500 tracking-widest uppercase">Bolívares</span>
                    <span className="text-xl font-black text-red-600 dark:text-red-400">{formatBs(cartTotalBs)} Bs</span>
                </div>
