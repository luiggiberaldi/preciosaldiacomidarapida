...
export default function CheckoutModal({
    ...
    const handleFastCash = useCallback((amount) => {
        triggerHaptic && triggerHaptic();
        const current = parseFloat(barValues['efectivo_usd']) || 0;
        setBarValues(prev => ({ ...prev, efectivo_usd: (current + amount).toString() }));
    ...
    }, [barValues, paymentMethods, effectiveRate, onConfirmSale, triggerHaptic, changeUsdGiven, changeBsGiven, changeUsd, deliveryType]);
    ...
    const sectionStyles = {
        USD: {
            ...
            inputBorder: 'border-amber-200 dark:border-amber-800 focus:border-red-500 focus:ring-red-500/20',
            inputActive: 'border-red-400 dark:border-red-600 bg-amber-50 dark:bg-amber-950/30',
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain pb-28">
                ...
                <div className="px-4 py-4 bg-gradient-to-b from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
                    ...
                    <div className="text-center mb-4">
                        ...
                        <span className="block text-sm font-bold text-red-600 dark:text-red-400 mt-0.5">Bs {formatBs(cartTotalBs)}</span>
                    </div>

                    {/* DELIVERY TYPE SELECTOR */}
                    <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-xl shadow-inner max-w-[240px] mx-auto">
                        <button
                            onClick={() => { triggerHaptic && triggerHaptic(); setDeliveryType('LOCAL'); }}
                        ...
                        <button
                            onClick={() => { triggerHaptic && triggerHaptic(); setDeliveryType('LLEVAR'); }}
                ...
                <div className="px-3 py-2">
                    <div className={`p-3.5 rounded-xl border-2 transition-all ${isPaid
                        ...
                        <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isPaid ? 'text-red-500' : 'text-orange-500'
                            }`}>
                            {isPaid ? 'Vuelto' : 'Resta por Cobrar'}
                        </p>
                        <div className="flex items-end justify-between">
                            <span className={`text-2xl font-black ${isPaid ? 'text-red-600 dark:text-red-400' : 'text-orange-600 dark:text-orange-400'
                                }`}>
                                ${isPaid ? changeUsd.toFixed(2) : remainingUsd.toFixed(2)}
                            </span>
                            <span className={`text-sm font-bold ${isPaid ? 'text-red-500' : 'text-orange-500'
                                }`}>
                            ...
                            <div className="mt-3 pt-3 border-t border-amber-200 dark:border-amber-800 space-y-2">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <div className="relative flex-1">
                                        ...
                                        />
                                        <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[9px] font-black text-red-600 bg-amber-50 dark:bg-amber-900/30 px-1 py-0.5 rounded">USD</span>
                                    </div>
                            ...
                            <div className="mt-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-lg max-h-40 overflow-y-auto">
                                ...
                                {customers.map(c => (
                                    ...
                                    >
                                        {c.name}
                                        {c.deuda !== 0 && (
                                            <span className={`ml-2 text-xs font-bold ${c.deuda > 0 ? 'text-red-500' : 'text-red-500'}`}>
