...
export default function CalculatorInput({
  ...
  const displayCurrency = ["BCV", "USD", "$ BCV", "Dolar"].includes(currency)
    ? "USD"
    : currency;

  return (
    <div
      className={`${compact ? "p-2" : "p-4"} bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-200 dark:border-slate-700 relative transition-all focus-within:border-brand focus-within:ring-1 focus-within:ring-brand/20`}
    >
      {/* Etiqueta Superior */}
      <span
        className={`${compact ? "text-[8px] mb-0.5" : "text-[10px] mb-1"} font-black text-slate-400 uppercase tracking-widest block`}
      >
        {label}
      </span>
