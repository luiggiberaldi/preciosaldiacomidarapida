...
export function useCalculator(rates) {
  ...
  useEffect(() => {
    ...
    if (lastEdited === "top") {
      ...
      const res = CurrencyService.calculateExchange(
        CurrencyService.safeParse(amountTop),
        rateFrom,
        rateTo,
      );
      ...
      const res = CurrencyService.calculateExchange(
        CurrencyService.safeParse(amountBot),
        rateTo,
        rateFrom,
      );
  ...
  const handleQuickAdd = (val) => {
    ...
    const finalVal =
      from === "VES" ? Math.ceil(newVal).toString() : newVal.toFixed(0);
    setAmountTop(finalVal);
    setLastEdited("top");
  };
