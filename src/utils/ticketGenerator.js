...
export async function generateTicketPDF(sale, bcvRate) {
  ...
  doc.setFontSize(7);
  doc.setTextColor(...MUTED);
  doc.text("Tasa BCV: Bs " + formatBs(rate) + " por $1", CX, y, {
    align: "center",
  });
  y += 8;
  ...
  if (showPayments) {
    ...
    if (hasFiado) {
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(6.5);
      doc.text("Bs " + formatBs(sale.fiadoUsd * rate), RIGHT, y, {
        align: "right",
      });
      y += 6;
    }
  ...
  doc.setFontSize(6);
  doc.setTextColor(...MUTED);
  doc.text("Comprobante digital · Sin valor fiscal", CX, y, {
    align: "center",
  });
  ...
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    navigator
      .share({ title: "Ticket #" + saleNum, files: [file] })
