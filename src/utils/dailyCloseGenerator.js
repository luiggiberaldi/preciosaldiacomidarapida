...
export async function generateDailyClosePDF({
  ...
  const H = 200 + paymentRows * 7 + topProdRows * 10 + saleRows * 22;
  ...
  if (paymentRows > 0) {
    ...
    Object.entries(paymentBreakdown).forEach(([methodId, data]) => {
      const label = data.label || getPaymentLabel(methodId);
      const val =
        data.currency === "USD"
  ...
  allSales.forEach((s) => {
    ...
    if (s.items && s.items.length > 0 && !isCanceled) {
      s.items.forEach((item) => {
        ...
        doc.text(`$${(item.priceUsd * item.qty).toFixed(2)}`, RIGHT, y, {
          align: "right",
        });
        y += 3.5;
      });
    ...
    if (methodStr && !isCanceled) {
      ...
      // Ref en Bs
      doc.text(`Ref: Bs ${formatBs(s.totalBs || 0)}`, RIGHT, y, {
        align: "right",
      });
      y += 3.5;
    }
  ...
  doc.setFontSize(6);
  doc.setTextColor(...MUTED);
  doc.text("Reporte generado automáticamente · Sin valor fiscal", CX, y, {
    align: "center",
  });
  ...
  if (navigator.canShare && navigator.canShare({ files: [file] })) {
    navigator
      .share({ title: `Cierre del Día ${dateStr}`, files: [file] })
