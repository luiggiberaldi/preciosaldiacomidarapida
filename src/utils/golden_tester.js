...
export const standardTests = [
  {
    ...
    validator: (aiResult, rates) => {
      ...
      if (!result)
        return { ok: false, msg: "ERROR: No hay campo convertedAmount" };
      const expected = Number(
        auditor.calculateExpected(100, "USDT", "USD", rates).toFixed(2),
      );
      const diff = Math.abs(result - expected);
      if (diff > 0.5)
        return {
          ok: false,
          msg: `ERROR: Discrepancia USD. Esperado ~${expected}, Recibido ${result}`,
        };
  ...
  },
  {
    ...
    validator: (aiResult) => {
      ...
      return {
        ok: false,
        msg: "ERROR: No detectó intención de análisis de imagen.",
      };
    },
  },
  {
    ...
    validator: (aiResult, rates) => {
      ...
      const expected = Math.ceil(
        auditor.calculateExpected(50, "EUR", "VES", rates),
      );
      const diff = Math.abs(result - expected);
      if (diff > 0.5)
        return {
          ok: false,
          msg: `ERROR: Discrepancia EUR. Esperado ~${expected}, Recibido ${result}`,
        };
  ...
  },
  {
    ...
    validator: (aiResult) => {
      ...
      return {
        ok: false,
        msg: "ERROR: Respuesta genérica, sin personalidad VIP.",
      };
    },
  },
  {
    ...
    validator: (aiResult, rates) => {
      ...
      if (diff <= tolerance)
        return {
          ok: true,
          msg: "Cálculo de efectivo consistente (Base USDT/Calle).",
        };
      return {
        ok: false,
        msg: `Diferencia detectada en efectivo. Esperado ~${expected} (Base USDT), Recibido ${result}.`,
      };
    },
  },
  {
    ...
    validator: (aiResult, rates) => {
      ...
      if (
        analysis.includes("brecha") ||
        rates.bcv.price / rates.bcv.price > 1
      ) {
        return { ok: true, msg: "Análisis de brecha detectado y coherente." };
      }
      return {
        ok: false,
        msg: "ERROR: El análisis no mencionó la brecha cambiaria.",
      };
    },
  },
  {
    ...
    validator: (aiResult) => {
      ...
      if (missing.length === 0)
        return { ok: true, msg: "JSON Schema v2 íntegro." };
      return {
        ok: false,
        msg: `ERROR: Campos faltantes en JSON: ${missing.join(", ")}`,
      };
    },
  },
  {
    ...
    validator: (aiResult, rates) => {
      const result = aiResult.convertedAmount;
      const expected = Math.ceil(
        auditor.calculateExpected(100, "USD", "VES", rates),
      );
      const diff = Math.abs(result - expected);
      if (diff < 5)
        return { ok: true, msg: "Pregunta trampa resuelta (Default a VES)." };
      return {
        ok: false,
        msg: `ERROR: Falló en pregunta trampa. Esperado ~${expected}, Recibido ${result}`,
      };
    },
  },
];
...
export const runAudit = async (rates, logCallback) => {
    ...
    try {
      ...
      if (validation.ok) {
        ...
        if (aiResult.textResponse)
          logCallback(
            `> Response: "${aiResult.textResponse.substring(0, 50)}..."`,
          );
      }
    } catch (e) {
  ...
  logCallback(
    `📊 Score Final: ${finalScore.toFixed(0)}% (${passed}/${standardTests.length})`,
  );

  if (finalScore === 100) {
    logCallback(
      "🏆 SISTEMA CERTIFICADO: PreciosAlDía es apto para despliegue.",
    );
  } else {
    logCallback(
      "⚠️ ATENCIÓN: Se requieren ajustes en el prompt o lógica del sistema.",
    );
  }
};
